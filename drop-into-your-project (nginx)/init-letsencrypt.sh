...
function request_new_certificate {
  ...
  if [ -z "$creds" ]; then
    ...
    docker-compose run --rm --entrypoint " \
      ...
      --dns-dnsprovider-propagation-seconds 30
  fi
  echo
}
...
domains_list=(example.com www.example.com)
rsa_key_size=4096
data_path="./server/certbot"
dns_cred_path="/etc/letsencrypt/dnsprovider.ini" # comment out if not using a dns provider
email="someones@email.com" # Adding a valid address is strongly recommended
